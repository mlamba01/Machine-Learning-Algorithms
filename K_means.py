...
def visualize_clusters(Clustering, cluster_class, directory):
...
def finding_centre(centers, dataset, k):
...
def find_K_means(k):
	...
	while True:
		idx_cluster = finding_centre(centroids, X_train, k)			# Seperate data into clusters by finding nearest center
		k_means = []
...
def K_means_Clustering(k):
	Clustering, MSE, Avg_MSE, MSS, idx_cluster = best_cluster(k)	# Get best clustering for given k
	print('\n Results for (K =', k,') :\n\n\tNo. of clusters = ', len(Clustering), '\n\tAverage Mean Square Error = ', Avg_MSE, '\n\tMean Square Seperation = ', MSS, '\n')
	cluster_class = assign_class(idx_cluster)			# Assign classes to each cluster center
	print('\tAssigned classes to clusters : ', cluster_class)
	test_set_clusters = finding_centre(Clustering, X_test, len(cluster_class))		# Cluster test data by finding nearest center for each vector
	...
	visualize_clusters(Clustering, cluster_class, '-'.join(['K',str(k)]))
